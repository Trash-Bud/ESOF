...
AppState appReducers(AppState state, dynamic action) {
  ...
  } else if (action is SetCurrentAccountAction) {
    return setCurrentAccount(state, action);
  } else if (action is SetNotificationsAction) {
    return setNotifications(state, action);
  } else if (action is SetPrintsAction) {
    return setPrints(state, action);
  } else if (action is SetExamsStatusAction) {
    return setExamsStatus(state, action);
  } else if (action is SetCurrentAccountStatusAction) {
    return setCurrentAccountStatus(state, action);
  } else if (action is SetNotificationsStatusAction) {
    return setNotificationsStatus(state, action);
  } else if (action is SetPrintsStatusAction) {
    return setPrintsStatus(state, action);
  ...
  } else if (action is UpdateFavoriteSPCards) {
    return updateFavoriteSPCards(state, action);
  ...
  } else if (action is SetSigarraPayPageEditingMode) {
    return setSigarraPayPageEditingMode(state, action);
  ...
  } else if (action is SetExpensesFilter){
    return setExpensesFilter(state,action);
  } else if (action is SetExpensesOrder){
    return setExpensesOrder(state,action);
  } else if (action is SetExpensesOrderCriteria){
    return setExpensesOrderCriteria(state,action);
  }
...
AppState setCurrentAccount(AppState state, SetCurrentAccountAction action) {
  Logger().i('setting current account: ' + action.toString());
  return state.cloneAndUpdateValue('currentAccount', action.currentAccount);
}

AppState setNotifications(AppState state, SetNotificationsAction action) {
  Logger().i('setting notifications: ' + action.toString());
  return state.cloneAndUpdateValue('notifications', action.notifications);
}

AppState setPrints(AppState state, SetPrintsAction action) {
  Logger().i('setting prints: ' + action.toString());
  return state.cloneAndUpdateValue('prints', action.print);
}
...
AppState setCurrentAccountStatus(AppState state, SetCurrentAccountStatusAction action) {
  Logger().i('setting current account status: ' + action.status.toString());
  return state.cloneAndUpdateValue('currentAccountStatus', action.status);
}


AppState setNotificationsStatus(AppState state, SetNotificationsStatusAction action) {
  Logger().i('setting notifications status: ' + action.status.toString());
  return state.cloneAndUpdateValue('notificationsStatus', action.status);
}

AppState setPrintsStatus(AppState state, SetPrintsStatusAction action) {
  Logger().i('setting prints status: ' + action.status.toString());
  return state.cloneAndUpdateValue('printsStatus', action.status);
}
...
AppState updateFavoriteSPCards(AppState state, UpdateFavoriteSPCards action) {
  return state.cloneAndUpdateValue('favoriteSPCards', action.favoriteSPCards);
}
...
AppState setSigarraPayPageEditingMode(AppState state,
    SetSigarraPayPageEditingMode action) {
  Logger().i('setting SigarraPay page editing mode to ' + action.state.toString());
  return state.cloneAndUpdateValue('sigarraPayPageEditingMode', action.state);
}
...
AppState setExpensesFilter(AppState state, SetExpensesFilter action) {
  Logger().i('setting expense type filter to ' + action.filteredExpenses.toString());
  return state.cloneAndUpdateValue('filteredExpenses', action.filteredExpenses);
}

AppState setExpensesOrder(AppState state, SetExpensesOrder action) {
  Logger().i('setting expense order to ' + action.orderedExams.toString());
  return state.cloneAndUpdateValue('orderedExpenses', action.orderedExams);
}

AppState setExpensesOrderCriteria(AppState state, SetExpensesOrderCriteria action) {
  Logger().i('setting expense order criteria to ' + action.orderedExpensesCriteria.toString());
  return state.cloneAndUpdateValue('orderedExpensesCriteria', action.orderedExpensesCriteria);
}
