...
class AppSharedPreferences {
  ...
  static final String favoriteSPCards = 'favorite_SigarraPay_cards';
  static final List<FAVORITE_SP_WIDGET_TYPE> defaultFavoriteSPCards = [
    FAVORITE_SP_WIDGET_TYPE.expenses,
    FAVORITE_SP_WIDGET_TYPE.prints,
    FAVORITE_SP_WIDGET_TYPE.bankReferences,
  ];
  ...
  static final String orderedExpenseTypes = 'ordered_expense_types';
  static final List<String> defaultOrderedExpenseTypes =
  Expense.getOrderedExpenseTypes();

  static final String orderedExpenseCriteria = 'ordered_expense_criteria';
  static final List<String> defaultOrderedExpenseCriterion =
  ...
  /// Replaces the user's favorite widgets with [newFavorites].
  static saveFavoriteSPCards(List<FAVORITE_SP_WIDGET_TYPE> newFavorites) async {
    final prefs = await SharedPreferences.getInstance();
    prefs.setStringList(
        favoriteSPCards, newFavorites.map((a) => a.index.toString()).toList());
  }
  ...
  /// Returns a list containing the user's favorite widgets.
  static Future<List<FAVORITE_SP_WIDGET_TYPE>> getFavoriteSPCards() async {
    final prefs = await SharedPreferences.getInstance();
    final List<String> storedSPFavorites = prefs.getStringList(favoriteSPCards);
    if (storedSPFavorites == null) return defaultFavoriteSPCards;
    return storedSPFavorites
        .map((i) => FAVORITE_SP_WIDGET_TYPE.values[int.parse(i)])
        .toList() ??
        defaultFavoriteSPCards;
  }


  static saveOrderedExpenses(String newOrderedExpenseType) async {
    final prefs = await SharedPreferences.getInstance();

    prefs.setString(orderedExpenseTypes, newOrderedExpenseType);
  }

  static saveOrderedExpensesCriteria(String newOrderedExpenseCriteria) async {
    final prefs = await SharedPreferences.getInstance();

    prefs.setString(orderedExpenseCriteria, newOrderedExpenseCriteria);
  }
  ...
  static Future<String> getOrderedExpenses() async {
    final prefs = await SharedPreferences.getInstance();
    final String storedOrderedExpenseType =
    prefs.getString(orderedExpenseTypes);

    if (storedOrderedExpenseType == null) {
      return defaultOrderedExpenseTypes[0];
    }
    return storedOrderedExpenseType;
  }

  static Future<String> getOrderedExpensesCriteria() async {
    final prefs = await SharedPreferences.getInstance();
    final String storedOrderedExpenseCriteria =
    prefs.getString(orderedExpenseCriteria);

    if (storedOrderedExpenseCriteria == null) {
      return defaultOrderedExpenseCriterion[0];
    }
    return storedOrderedExpenseCriteria;
  }
