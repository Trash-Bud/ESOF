...
import 'package:uni/controller/parsers/parse_prints.dart';
...
Future loadRemoteUserInfoToState(Store<AppState> store) async {
  ...
  store.dispatch(getUserNotifications(notifications));

  final Tuple2<String, String> userPersistentInfo =
      await AppSharedPreferences.getPersistentUserInfo();
  userInfo.future.then((value) {
    store.dispatch(getUserExams(exams, ParserExams(), userPersistentInfo));
    store.dispatch(getUserCurrentAccount(currentAccount, ParserCurrentAccount(), userPersistentInfo));
    store.dispatch(getUserPrints(prints, ParserPrints(), ParserPrintingAccounts(), userPersistentInfo));
    store.dispatch(getUserSchedule(schedule, userPersistentInfo));
  });

  final allRequests = Future.wait([
    exams.future,
    printingAccounts.future,
    currentAccount.future,
    prints.future,
    schedule.future,
    printBalance.future,
    fees.future,
...
void loadLocalUserInfoToState(store) async {
  ...
  store.dispatch(
      UpdateFavoriteSPCards(await AppSharedPreferences.getFavoriteSPCards()));
  store.dispatch(SetExamFilter(await AppSharedPreferences.getFilteredExams()));

  store.dispatch(SetExpensesOrder(await AppSharedPreferences.getOrderedExpenses()));
  store.dispatch(SetExpensesOrderCriteria(await AppSharedPreferences.getOrderedExpensesCriteria()));
  store.dispatch(
  ...
  if (userPersistentInfo.item1 != '' && userPersistentInfo.item2 != '') {
    ...
    store.dispatch(updateStateBasedOnLocalUserCurrentAccount());
    ...
    store.dispatch(updateStateBasedOnLocalUserNotifications());
